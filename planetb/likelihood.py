...
core_mass_grid = np.linspace(3.8, 5.2, 16)
core_radius_grid = np.linspace(1.39, 1.58, 16)

for i in range(0,16):
    for j in range(0,16):
        for k in range(0,64):
            for l in range(0,64):
                if initial_mass[i,j,k,l] > 1:
                    initial_mass[i,j,k,l]=np.log10((initial_mass[i,j,k,l]-core_mass_grid[i]*5.97219e27)/(core_mass_grid[i]*5.97219e27))
...
def loglike(p):
	...
	kep36_mass = 4.45 * 5.97219e27
	kep36_radius = 1.486 * 6.4e8
	kep36_mass_err = 0.04 * kep36_mass
	kep36_radius_err = 0.008 * kep36_radius
...
pstart=fmin(negloglike, (4.45,   1.486,   np.log10(0.03), np.log10(1.01441188e+07)))
print pstart
